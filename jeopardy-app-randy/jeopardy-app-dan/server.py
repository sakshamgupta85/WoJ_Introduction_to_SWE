...
def get_board_html(board):
    ...
    to_return = "<center><table id='jeoBoard'><tbody><tr>"
...
def make_new_wheel(board):
    ...
    return to_return#["Lose a Turn", "Free Turn", "Free Turn"]
...
def check_if_over(curr_game):
    if curr_game.spins_remaining == 0:
            if curr_game.curr_round == 1:
                # progress the game to the next round
                # reset teh board, the wheel
                # save current round totals to game totals
                ...
                curr_game.board = make_new_board(2)
                curr_game.wheel = make_new_wheel(curr_game.board)
                curr_game.curr_round = 2
                curr_game.spins_remaining = curr_game.spins_per_round
                return False, True, {}
                
            else:
                # the game is over!
                # return the results to the front end
                # add the round totals to the game totals
                # return the larger game total
                players = []
                scores = []
                for player in curr_game.round_total.keys():
                    players.append(player)
                    scores.append(curr_game.round_total[player] + curr_game.game_total[player])
                if scores[0] < scores[1]:
                    winner = players[1]
                else:
                    winner = players[0]
                print("GAME OVER ~~~~~~~~~~~~~~~", winner)
                return True, True, {"game_over": True, "winner": winner}
    else:
...
async def spin(request: Request):
    ...
    if (
        ...
        game_is_over, round_is_over, response = check_if_over(curr_game)
        if game_is_over:
            return response
        if round_is_over:
            # board: board,
            ...
            # curr_question: null,
            # scores:scores,
            board = get_board_html(curr_game.board)
            currently_playing = get_players_html(curr_game.players)
            curr_scores = get_scores_html(curr_game.round_total, curr_game.game_total)
            return {"round_over":True, "board":board, "players": currently_playing, "uid": curr_id,\
                "spins_rem": curr_game.spins_remaining, "cp": curr_game.curr_player,"scores":curr_scores}
        ...
        if cat in NON_QUESTION_CATEGORIES:
            ...
            if cat == 'Lose a Turn':
                if curr_game.player_free_turns[curr_game.curr_player] < 1:
                    curr_game.next_player()
                    return {"cat":cat, "status":"no_tokens", "cp":curr_game.curr_player, "spins_rem": curr_game.spins_remaining, "game_over":False} 
                else:
                    return {"cat":cat, "status":"tokens_available", "cp":curr_game.curr_player, "spins_rem": curr_game.spins_remaining, "game_over":False} 
            if cat == 'Free Turn':
        ...
        return {"board": board, "players": currently_playing, "uid": curr_id, "curr_q": question_to_send,\
             "scores":curr_scores, "spins_rem": curr_game.spins_remaining, "cp": curr_game.curr_player, "cat":cat, "game_over":False}
...
async def guess(request: Request):
    ...
    if (
        ...
        return {"board": board, "players": currently_playing, "uid": curr_id, "curr_q": curr_question.question, \
            "scores":curr_scores_resp, "spins_rem": curr_game.spins_remaining, "cp":curr_game.curr_player}
